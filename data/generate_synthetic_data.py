...
from utils import load_gatech_data, interpolate_data, joint_peaks, extract_locomujoco_data, to_training_format

os.environ['MUJOCO_GL'] = 'egl'

def get_control_points(cycle, normalize=False):
    points = [round(len(cycle) * 0.05 * x) for x in range(20)]
    points.append(len(cycle) -1)
    if normalize:
        points = [point*100/len(cycle) for point in points]
    return points

def linear_transformation(control_gait_phase, control_angle, models, joint, speed, root_speed):
    assert len(control_gait_phase) == models[joint]['num_control_point']
    gait_phase_intercepts = np.array(models[joint]['intercepts']['Gait Phase'])
    gait_phase_slopes = np.array(models[joint]['slopes']['Gait Phase'])
    angle_intercepts = np.array(models[joint]['intercepts']['Angle'])
    angle_slopes = np.array(models[joint]['slopes']['Angle'])
    ...
    for speed in tqdm(speeds, desc='Generating Synthetic Data'):
        ...
        for joint in cycles:
            ...
            for cycle in cycles[joint]:
                ...
                transformed_gait_phase, transformed_angle = transform(control_gait_phase, control_angle, linear_models, joint[2:], speed, root_speed)
...
if __name__ == "__main__":
    data, joints = load_gatech_data(root='dataset_csv')
    ...
    for joint in joints:
        for speed in speeds:
            ...
            control_xs = joint_peaks(avg_cycle, normalize=True)
    ...
    # for eval
    with open('locomujoco_13_speeds_dataset_unformatted.pkl', 'wb') as f:
        pickle.dump(generated_data, f)

    format_data = to_training_format(generated_data)
    # for training
    with open('locomujoco_13_speeds_dataset.pkl', 'wb') as f:
